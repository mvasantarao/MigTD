...
pub fn spdm_responder<'a, T: AsyncRead + AsyncWrite + Unpin + Send + Sync + 'static>(
    stream: T,
) -> Result<(ResponderContextEx<'a>, SpdmDeviceIoArc<T>), SpdmStatus> {
...
pub fn handle_exchange_pub_key_req(
    ...
    if vdm_request.element_count != VDM_MESSAGE_EXCHANGE_PUB_KEY_REQ_ELEMENT_COUNT {
        error!(
    ...
    let vdm_exchange_pub_key = VdmMessage {
        ...
        element_count: VDM_MESSAGE_EXCHANGE_PUB_KEY_RSP_ELEMENT_COUNT,
    };
...
pub fn handle_exchange_mig_attest_info_req(
    ...
    if vdm_request.element_count != VDM_MESSAGE_EXCHANGE_MIGRATION_ATTEST_INFO_REQ_ELEMENT_COUNT {
        error!(
            "Invalid VDM message element_count: {:x?}\n",
            vdm_request.element_count
        );
...
pub fn handle_exchange_mig_info_req(
    ...
    if vdm_request.element_count != VDM_MESSAGE_EXCHANGE_MIGRATION_INFO_REQ_ELEMENT_COUNT {
        error!(
            "Invalid VDM message element_count: {:x?}\n",
            vdm_request.element_count
        );
    ...
    if mig_export_version_element.element_type != VdmMessageElementType::MigrationExportVersion
        || mig_export_version_element.length != VDM_MESSAGE_MIGRATION_EXPORT_VERSION_SIZE
    {
    ...
    let vdm_exchange_mig_info = VdmMessage {
        ...
        element_count: VDM_MESSAGE_EXCHANGE_MIGRATION_INFO_RSP_ELEMENT_COUNT,
    };
    cnt += vdm_exchange_mig_info
        .encode(&mut writer)
        .map_err(|_| SPDM_STATUS_BUFFER_FULL)?;

    //Migration Import Version
    let mig_import_version_element = VdmMessageElement {
        element_type: VdmMessageElementType::MigrationImportVersion,
        length: VDM_MESSAGE_MIGRATION_IMPORT_VERSION_SIZE,
    };
    ...
    //Backward Migration Session Key
    let backward_mig_session_key_element = VdmMessageElement {
        element_type: VdmMessageElementType::BackwardMigrationSessionKey,
        length: VDM_MESSAGE_BACKWARD_MIGRATION_SESSION_KEY_SIZE,
    };
    cnt += backward_mig_session_key_element
        .encode(&mut writer)
        .map_err(|_| SPDM_STATUS_BUFFER_FULL)?;
    cnt += writer
...
#[cfg(all(feature = "main", feature = "policy_v2", feature = "vmcall-raw"))]
pub fn handle_exchange_rebind_attest_info_req(
    responder_context: &mut ResponderContext,
    session_id: Option<u32>,
    vdm_request: &VdmMessage,
    reader: &mut Reader<'_>,
    ...
    use crate::ratls::gen_tdreport;
    // Rebinding messages must be handled only when rebinding info is set.
    let spdm_responder_ex = unsafe { upcast_mut(responder_context) };
    if !matches!(
        &spdm_responder_ex.info,
        ResponderContextExInfo::RebindInformation(_)
    ) {
        error!("Rebinding info is not set in responder context.\n");
        return Err(SPDM_STATUS_INVALID_MSG_FIELD);
    }
    ...
    if vdm_request.op_code != VdmMessageOpCode::ExchangeRebindAttestInfoReq {
        error!("Invalid VDM message op_code: {:x?}\n", vdm_request.op_code);
        return Err(SPDM_STATUS_INVALID_MSG_FIELD);
    }
    ...
    let td_report_src = reader
        .take(vdm_element.length as usize)
        .ok_or(SPDM_STATUS_INVALID_MSG_SIZE)?;
    let td_report_src_vec = td_report_src.to_vec();
    ...
    let vdm_exchange_attest_info = VdmMessage {
        major_version: VDM_MESSAGE_MAJOR_VERSION,
        minor_version: VDM_MESSAGE_MINOR_VERSION,
        op_code: VdmMessageOpCode::ExchangeRebindAttestInfoRsp,
        element_count: VDM_MESSAGE_EXCHANGE_REBIND_ATTEST_INFO_RSP_ELEMENT_COUNT,
    };
    ...
    //TD report dst
    let td_report_dst = gen_tdreport(&report_data[..report_data_prefix_len + th1_len])
    ...
    cnt += tdreport_element
        ...
        .map_err(|_| SPDM_STATUS_BUFFER_FULL)?;
    cnt += writer
        .extend_from_slice(td_report_dst_bytes)
        .ok_or(SPDM_STATUS_BUFFER_FULL)?;
...
#[cfg(all(feature = "main", feature = "policy_v2", feature = "vmcall-raw"))]
pub fn handle_exchange_rebind_info_req(
    responder_context: &mut ResponderContext,
    session_id: Option<u32>,
    vdm_request: &VdmMessage,
    reader: &mut Reader<'_>,
    ...
    if vdm_request.op_code != VdmMessageOpCode::ExchangeRebindInfoReq {
        error!("Invalid VDM message op_code: {:x?}\n", vdm_request.op_code);
        return Err(SPDM_STATUS_INVALID_MSG_FIELD);
    }
    if vdm_request.element_count != VDM_MESSAGE_EXCHANGE_REBIND_INFO_ELEMENT_REQ_COUNT {
        error!(
            "Invalid VDM message element_count: {:x?}\n",
            vdm_request.element_count
        );
    ...
    let vdm_exchange_mig_info = VdmMessage {
        major_version: VDM_MESSAGE_MAJOR_VERSION,
        minor_version: VDM_MESSAGE_MINOR_VERSION,
        op_code: VdmMessageOpCode::ExchangeRebindInfoRsp,
        element_count: VDM_MESSAGE_EXCHANGE_REBIND_INFO_ELEMENT_RSP_COUNT,
    };
    cnt += vdm_exchange_mig_info
        .encode(&mut writer)
        .map_err(|_| SPDM_STATUS_BUFFER_FULL)?;
